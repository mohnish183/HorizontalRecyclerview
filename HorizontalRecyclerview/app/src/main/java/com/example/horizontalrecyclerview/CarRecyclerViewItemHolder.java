...
public class CarRecyclerViewItemHolder extends RecyclerView.ViewHolder {
     ...
     * @since 13 June 2019
     */
    public static class CouponsTab extends Fragment {
        ...
        public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
            ...
            ArrayList<CarRecyclerViewItem.CompanyOfferAndCoupon> couponsData = new ArrayList<>();

            // coupon and offer data that is passed from CompanyCouponActivity is then sorted
            ArrayList<CarRecyclerViewItem.CompanyOfferAndCoupon> allData = (ArrayList<CarRecyclerViewItem.CompanyOfferAndCoupon>) getArguments().getSerializable("dataset");
            for (int i = 0; i < allData.size(); i++) {
                // adding all COUPON data, hence omitting OFFER data
                if (allData.get(i).getType() == CarRecyclerViewItem.CompanyOfferAndCoupon.COUPON)
        ...
        public class CouponsTabListAdapter extends BaseAdapter {
            private ArrayList<CarRecyclerViewItem.CompanyOfferAndCoupon> listOfOffersAndCoupons;

            /**Constructor*/
            CouponsTabListAdapter(ArrayList<CarRecyclerViewItem.CompanyOfferAndCoupon> listOfOffersAndCoupons) {
            ...
            public View getView(int position, View convertView, ViewGroup parent) {
                CarRecyclerViewItem.CompanyOfferAndCoupon couponOrOffer = listOfOffersAndCoupons.get(position);
